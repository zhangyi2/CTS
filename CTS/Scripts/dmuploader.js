...
(function($) {
  ...
  var defaults = {
    url: document.URL,
    method: 'POST',
    operate: 'uploadmedia',//add 2015/10/10
    extraData: {},
    maxFileSize: 0,
    maxFiles: 0,
    allowedTypes: '*',
  ...
  DmUploader.prototype.reset = function () {//add 2015/10/10
      var widget = this;
  ...
  DmUploader.prototype.deleteFile = function (pos) {
      var widget = this;
      if (pos >= 0 && pos <= widget.queue.length) {
          widget.queue.splice(pos, 1);
          widget.queuePos--;
          //widget.queueRunning = false;
          if (this.settings.onDelFile != null) {
              this.settings.onDelFile.call(this.element, pos);
          }
          return;
      }
  }
    ...
    $.ajax({
      ...
      success: function (data, message, xhr){
          ...
          try {
              ...
              widget.settings.onUploadError.call(widget.element, widget.queuePos, e.message);
          }
      },
      error: function (xhr, status, errMsg) {
          ...
          try {
              ...
              widget.settings.onUploadError.call(widget.element, widget.queuePos, e.message || errMsg);
          }
      },
      complete: function(xhr, textStatus){
          //if (xhr.status == 200) {
          //    var d = JSON.parse(xhr.responseText);
          //    widget.settings.onUploadSuccess.call(widget.element, widget.queuePos, d);
          //}
          //else {
          //    widget.settings.onUploadError.call(widget.element, widget.queuePos, xhr);
          //}
          widget.processQueue();
      }
    });
